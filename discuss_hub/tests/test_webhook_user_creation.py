...
class TestWebhookUserCreation(HttpCase):
    ...
    def test_webhook_payload_creates_portal_user(self):
        ...
        try:
            ...
            print(f"\n✅ Test passed! User created: {user.name} (login: {user.login})")
            print(f"   Groups: {user.group_ids.mapped('name')}")
            
        except Exception as e:
            print(f"\n❌ Error during webhook user creation: {e}")
    ...
    def test_webhook_payload_with_existing_partner_creates_user(self):
        ...
        webhook_payload = {
            ...
            "data": {
                ...
                "message": {
                    "conversation": "Message from existing partner"
                },
            },
        ...
        try:
            ...
            print(f"\n✅ User created for existing partner: {user.name}")
            print(f"   Groups: {user.group_ids.mapped('name')}")
            
        except Exception as e:
            print(f"\n❌ Error creating user for existing partner: {e}")
            print(f"   Error type: {type(e).__name__}")
            raise
    ...
    def test_manual_user_creation_with_groups(self):
        ...
        try:
            ...
            print(f"✅ Method 1 SUCCESS: {user1.name}")
            print(f"   Groups: {user1.group_ids.mapped('name')}")
        except Exception as e:
        ...
        try:
            ...
            print(f"   User created: {user2.name}")
            
            # Now try to write group_ids
            user2.write({"group_ids": [(6, 0, [portal_group.id])]})
            print(f"✅ Method 2 SUCCESS: {user2.name}")
            print(f"   Groups: {user2.group_ids.mapped('name')}")
        except Exception as e:
        ...
        try:
            ...
            print(f"   User created: {user3.name}")
            
            # Try with sudo
            user3.sudo().write({"group_ids": [(6, 0, [portal_group.id])]})
            print(f"✅ Method 3 SUCCESS: {user3.name}")
            print(f"   Groups: {user3.group_ids.mapped('name')}")
        except Exception as e:
        ...
        try:
            ...
            print(f"✅ Method 4 SUCCESS: {user4.name}")
            print(f"   Groups: {user4.group_ids.mapped('name')}")
        except Exception as e:
            print(f"❌ Method 4 FAILED: {type(e).__name__}: {e}")
